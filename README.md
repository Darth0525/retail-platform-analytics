...
| [`03_customer_value_and_behavior`](analysis/03_customer_value_and_behavior.ipynb) | Who are the customers (and why RFM fails)? | ~97% buy once, so RFM has no traction (Frequency degenerate, Monetary circular); customer value is instead driven by **ticket size** (corr ≈0.90), not quantity — the premium price-point third drives ~65% of revenue |
