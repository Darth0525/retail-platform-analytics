...
| [`03_customer_value_and_behavior`](analysis/03_customer_value_and_behavior.ipynb) | Who are the customers (and why RFM fails)? | ~97% buy once and ~90% of orders are single-item, so RFM collapses; customer value is driven by **ticket size** (corr ≈0.90), not quantity — the premium price-point third drives ~65% of revenue |
